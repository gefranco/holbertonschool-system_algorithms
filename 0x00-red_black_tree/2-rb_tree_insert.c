...
- if parent of new node is red, then check the color of uncle's new node
	...
	- if color is red then recolor & check if parent's parent of new node is not root, if yes recolor & recheck	 
*/
...
int left_rotation(rb_tree_t *node, rb_tree_t **tree);
...
rb_tree_t *rb_tree_insert(rb_tree_t **tree, int value)
{
	...
	/*node = rb_tree_node(*tree, value, RED);
	(*tree)->left = node;*/
	/*if (value == 128)
		return NULL;
	*/
	...
	/*if (node->parent->color == RED && get_uncle(node) && get_uncle(node)->color == RED)
		recolor(node, &*tree);
	*/
	if (check_reds(node))
	{
...
int recolor(int recolor, rb_tree_t *node)
{
	...
	/*if (get_uncle(node) != NULL ) 
		get_uncle(node)->color = (get_uncle(node)->color == RED) ?
		BLACK : RED; 
	*/
	return 1;
}
